...
function stringifyValue(value, quote, indentation, indentString, newlines) {
    ...
    if (Array.isArray(value)) {
        var elements = value.map(function (element) {
            return stringifyValue(element, quote, indentation + indentString, indentString, true);
        });
        if (newlines) {
    ...
    if (type === 'object') {
        ...
        var properties = keys.map(function (key) {
            return stringifyProperty(key, value[key], quote, indentation + indentString, indentString, newlines);
        });
        if (newlines) {
            return ("{" + (indentation + indentString) +
...
function patch(str, value) {
    ...
    var quote = counts[SINGLE_QUOTE] > counts[DOUBLE_QUOTE] ?
        SINGLE_QUOTE :
        DOUBLE_QUOTE;
...
function patchValue(node, value, str, indentation, indentString, quote, newlines) {
    var type = typeof value;
    if (type === 'string') {
        if (node.type === 'Literal' && typeof node.value === 'string') {
    ...
    if (Array.isArray(value)) {
        if (node.type === 'ArrayExpression') {
            return patchArray(node, value, str, indentation, indentString, quote, newlines);
        }
        return stringifyValue(value, quote, indentation, indentString, newlines);
    }
    if (type === 'object') {
        if (node.type === 'ObjectExpression') {
            return patchObject(node, value, str, indentation, indentString, quote, newlines);
        }
        return stringifyValue(value, quote, indentation, indentString, newlines);
    }
...
function patchNumber(raw, value) {
    ...
    if (matchRadix && value % 1 === 0) {
        return ((matchRadix[1] === '+' && value >= 0 ? '+' : value < 0 ? '-' : '') +
            '0' + matchRadix[2] +
            Math.abs(value).toString(matchRadix[2] === 'b' || matchRadix[2] === 'B' ? 2 :
                matchRadix[2] === 'o' || matchRadix[2] === 'O' ? 8 :
                    matchRadix[2] === 'x' || matchRadix[2] === 'X' ? 16 : null));
    }
    var match = /^([-+])?(\.)?/.exec(raw);
    if (match && match[0].length > 0) {
        return ((match[1] === '+' && value >= 0 ? '+' : value < 0 ? '-' : '') +
            (match[2] ? String(Math.abs(value)).replace(/^0/, '') : String(Math.abs(value))));
    }
    return String(value);
}
function patchArray(node, value, str, indentation, indentString, quote, newlines) {
    if (value.length === 0) {
...
function patchObject(node, value, str, indentation, indentString, quote, newlines) {
    var keys = Object.keys(value);
    if (keys.length === 0) {
    ...
    for (; i < node.properties.length; i += 1) {
        ...
        if (propertyValue !== undefined) {
            ...
            patched += patchValue(property.value, propertyValue, str, indentation, indentString, quote, newlinesInsideValue);
            started = true;
        }
        c = property.end;
    }
...
function guessIndentString(str) {
    ...
    lines.forEach(function (line) {
        ...
        if (whitespace$$1.length === line.length)
            return;
        if (whitespace$$1[0] === '\t') {
            tabs += 1;
        }
...
export { parse, evaluate, patch, stringify };
