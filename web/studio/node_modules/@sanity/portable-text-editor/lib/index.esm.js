...
function createWithSchemaTypes(types) {
  return function withSchemaTypes(editor) {
    ...
    editor.normalizeNode = entry => {
      const [node, path] = entry;
      if (Text.isText(node) && path.length === 2 && node._type === void 0) {
...
function Synchronizer(props) {
  ...
  const flush = useCallback(flushFn => {
...
const PortableTextEditable = forwardRef(function PortableTextEditable2(props, forwardedRef) {
  ...
  const handleDOMChange = useCallback(() => {
    ...
    const currentSelection = slateEditor.selection;
    try {
      newDomRange = slateEditor.selection && ReactEditor.toDOMRange(slateEditor, slateEditor.selection);
    } catch (error) {
