...
function createWithSchemaTypes(types$1) {
  return function withSchemaTypes(editor) {
    ...
    editor.normalizeNode = entry => {
      const [node, path] = entry;
      if (slate.Text.isText(node) && path.length === 2 && node._type === void 0) {
...
function Synchronizer(props) {
  ...
  const flush = React.useCallback(flushFn => {
  ...
  }, [change$, onChange, onFlushPendingPatches, onFlushPendingPatchesThrottled, slateEditor, syncValue]);
...
const PortableTextEditable = React.forwardRef(function PortableTextEditable2(props, forwardedRef) {
  ...
  const handleDOMChange = React.useCallback(() => {
    ...
    const currentSelection = slateEditor.selection;
    try {
      newDomRange = slateEditor.selection && slateReact.ReactEditor.toDOMRange(slateEditor, slateEditor.selection);
    } catch (error) {
      removeAllDocumentSelectionRanges(true);
      slate.Transforms.deselect(slateEditor);
      slate.Transforms.select(slateEditor, [0, 0]);
      slateEditor.onChange();
      debug("Could not resolve selection (".concat(JSON.stringify(currentSelection), "), unselecting"));
    }
  }, [slateEditor]);
  React.useEffect(() => {
